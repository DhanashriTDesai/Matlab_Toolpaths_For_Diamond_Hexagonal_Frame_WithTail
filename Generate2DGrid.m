...
switch CellType
    case 'StretchDominatedDiamond'
        ...
        nx(nn+1)=nx(1); ny(nn+1)=ny(1)-lh; 
        nx(nn+2)=nx(nn); ny(nn+2)=ny(nn+1); 
        nx(nn+3)=nx(nn+2); ny(nn+3)=ny(nn)+lh;
        nx(nn+4)=nx(nn+1); ny(nn+4)=ny(nn+3);

        lhD=abs(nx(nn+1)-nx(nn+2)); 
        lvD=abs(ny(nn+1)-ny(nn+4));
